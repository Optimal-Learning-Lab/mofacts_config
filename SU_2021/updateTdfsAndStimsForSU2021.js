...
for(let i=0;i<tdfsJSON.length;i++){
    ...
    if(summerFiles.findIndex(x => x===tdf.fileName)!=-1){
        tdf.fileName = tdf.fileName.replace("SP_2021","SU_2021");
        tdf.tdfs.tutor.setspec[0].stimulusfile = [tdf.tdfs.tutor.setspec[0].stimulusfile[0].replace("SP_2021","SU_2021")];
...
for(let j=0;j<stimJSON.length;j++){
    ...
    if(summerFiles.findIndex(x => x===stim.fileName)!=-1){
        console.log('working',stim.fileName);
        stim.fileName = stim.fileName.replace("SP_2021","SU_2021");
